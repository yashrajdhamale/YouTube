...
import { faSearch, faMicrophone, faVideoSlash, faBell, faBars, faXmark } from '@fortawesome/free-solid-svg-icons';
...
export default function Nav(props) {
  ...
  const [darkTheme, setDarkTheme] = useState(true); // State for dark theme
  ...
  const toggleTheme = () => {
    ...
    props.onThemeToggle(!darkTheme); // Notify parent component of theme change
  };

  return (
    <div id="nav" className={darkTheme ? 'dark' : ''}>
      <div id="left">
        <FontAwesomeIcon icon={faBars} id='baricon' />
        {darkTheme ? <svg
						className="youtubeindark"
						viewBox="0 0 90 20"
					...
					>
						...
						<g>
							<g id="youtube-paths_yt4" fill="white">
					...
					</svg> :<img src={logo} alt="youtube" id='logoimg' style={{ userSelect: 'none' }} /> }
      </div>
        ...
        <div id="user-icon" onClick={toggleOptions}>
          ...
          {showOptions && (
            <div id="options-box">
              ...
              <div className="option" onClick={toggleTheme}>Change Theme</div>
            </div>
          )}
        </div>
      </div>
